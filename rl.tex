...
\begin{algorithm}
  \caption{General Q-Learning}\label{alg:general-q-learning}
  \begin{algorithmic}
    \State
    ...
    \For{each episode}
      ...
      \Repeat
        ...
        \State $a' \gets \epsilon-$greedy action for $s'$ from $\pi$ derived from $Q$
        \State $Q(s, a) \gets$ update using Q-learning rule \ref{eqn:q-update} or Sarsa rule \ref{eqn:sarsa-update}
        \State $s \gets s'$
        \State $a \gets a'$
      \Until $s$ is terminal
    \EndFor
