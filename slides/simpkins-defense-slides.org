...
- Reuse - reusing artifacts in the construction of new software
...
SQL provides reusable language constructs and semantics that map directly to relational data model, resulting in far less complex code. (The astute reader will notice that there's far more to the Java example above.)
...
- a state abstraction functions which defines the subset of the world the module cares about, and
